...
<template>
  <div class="bg-white">
          ...
          <nav aria-label="Progress" class="hidden sm:block">
            <ol role="list" class="flex space-x-4">
              <li v-for="(step, stepIdx) in steps" :key="step.name" class="flex items-center">
                <a
                  v-if="step.status === 'current'" :href="step.href" aria-current="page"
                  class="text-indigo-600"
                >{{ step.name }}</a>
                <a v-else :href="step.href">{{ step.name }}</a>
                <ChevronRightIcon
                  v-if="stepIdx !== steps.length - 1" class="ml-4 h-5 w-5 text-gray-300"
                  aria-hidden="true"
                />
              </li>
            </ol>
    ...
    <main class="relative grid grid-cols-1 mx-auto max-w-7xl gap-x-16 lg:grid-cols-2 xl:gap-x-48 lg:px-8">
      <h1 class="sr-only">
        Order information
      </h1>
      ...
      <section
        ...
        class="bg-gray-50 px-4 pb-10 pt-16 lg:col-start-2 lg:row-start-1 lg:bg-transparent lg:px-0 sm:px-6 lg:pb-16"
      >
        <div class="mx-auto max-w-lg lg:max-w-none">
          ...
          <dl class="hidden border-t border-gray-200 pt-6 text-sm font-medium text-gray-900 lg:block space-y-6">
      ...
      <form class="px-4 pb-36 pt-16 lg:col-start-1 lg:row-start-1 lg:px-0 sm:px-6 lg:pb-16">
        <div class="mx-auto max-w-lg lg:max-w-none">
          ...
          <div class="mt-10 border-t border-gray-200 pt-6 sm:flex sm:items-center sm:justify-between">
            <button
              type="submit"
              class="w-full border border-transparent rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm sm:order-last sm:ml-6 sm:w-auto hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 focus:ring-offset-gray-50"
            >
              Continue
            </button>
